...
void TypeChecker::invalidArity(std::string& name,
                               const Parsing::Position* start,
                               const Parsing::Position* end) {
    std::ostringstream message;
...
void TypeChecker::visit(AST::ECBoolean&) { this->lastType = booleanType; }

void TypeChecker::visit(AST::ECInteger&) { this->lastType = integerType; }
...
void TypeChecker::visit(AST::ISequence& sequence) {
    for (std::unique_ptr<AST::Instruction>& instruction :
