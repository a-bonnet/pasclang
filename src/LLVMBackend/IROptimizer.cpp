...
IROptimizer::IROptimizer(unsigned char optimizationLevel, llvm::Module* module, Message::BaseReporter* reporter) :
    optimizationLevel(optimizationLevel), module(module), reporter(reporter)
{
    this->functionPassManager = std::make_unique<llvm::legacy::FunctionPassManager>(module);

    if(this->optimizationLevel > 0)
    {
        this->functionPassManager->add(llvm::createPromoteMemoryToRegisterPass());
    ...
    if(this->optimizationLevel > 1)
    {
        // Add more passes
        // In the meantime...
        std::string noteMessage = "optimization levels higher than 1 are currently equivalent to -O1";
