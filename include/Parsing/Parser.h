...
class Parser {
  ...
  private:
    ...
    program(std::unique_ptr<AST::TableOfTypes>& types);
  ...
  public:
    ...
    void swapTokensList(std::vector<Token>& tokens) {
        std::swap(this->tokens, tokens);
    }
    std::unique_ptr<AST::Program> parse() {
        std::unique_ptr<AST::TableOfTypes> tot =
            std::make_unique<AST::TableOfTypes>();
        this->types = tot.get();
